...
class IntelligentTrafficLoadBasedStrategyUnitTest {
  ...
  void shouldSelectPhaseWithHighestScore() {
    ...
    TrafficLightPhase lowScorePhase =
        new TrafficLightPhase(Set.of(lowScoreLane), Map.of(lowScoreLane, TrafficLightType.REGULAR));

    TrafficLightPhase highScorePhase =
        new TrafficLightPhase(
            Set.of(highScoreLane), Map.of(highScoreLane, TrafficLightType.REGULAR));
    ...
    Map<LaneIdentifier, Queue<Vehicle>> laneQueues =
        Map.of(
            lowScoreLane, new LinkedList<>(List.of(low)),
            highScoreLane, new LinkedList<>(List.of(high)));

    try (MockedStatic<TrafficLightPhasesHolder> mocked =
        mockStatic(TrafficLightPhasesHolder.class)) {
      mocked
          .when(TrafficLightPhasesHolder::getAllTrafficLightPhases)
  ...
  void shouldConsiderOnlyLeadingRightTurnersInConditionalLane() {
    // given
    LaneIdentifier conditionalLane =
        new LaneIdentifier(Direction.NORTH, LaneType.STRAIGHT_OR_RIGHT);

    TrafficLightPhase conditionalPhase =
        new TrafficLightPhase(
            Set.of(conditionalLane), Map.of(conditionalLane, TrafficLightType.CONDITIONAL_RIGHT));
    ...
    Map<LaneIdentifier, Queue<Vehicle>> laneQueues = Map.of(conditionalLane, vehicles);
