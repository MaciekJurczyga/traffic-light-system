...
public class AddVehicleUnitTest {
  ...
  void setUp() {
    ...
    when(mockSimulationContext.getIntersectionTrafficController())
        .thenReturn(mockTrafficController);
    when(mockVehicle.getVehicleId()).thenReturn("some_id");
  }

  @Test
  @DisplayName(
      "Add vehicle command should add vehicle to context and to proper lane on intersection")
